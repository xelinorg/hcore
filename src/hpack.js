    ...
    if (readingFor >= 0){
      ...
      if (readingString) {
        if (readingAcc.decoded.length === 0) {
          ...
          const nameLengthArray = isHuffman ? maskHuffmanBit(readingAcc.name.nlength) : readingAcc.name.nlength
          const decodedLength = decodeInteger(nameLengthArray, 0x7f)
          if (decodedLength === readingAcc.name.octets.length) {
            const name = isHuffman ? readHuffman(readingAcc.name.octets): readingAcc.name.octets
          ...
          const valueLengthArray = isHuffman ? maskHuffmanBit(readingAcc.value.vlength) : readingAcc.value.vlength
          const decodedLength = decodeInteger(valueLengthArray, 0x7f)
          if (decodedLength === readingAcc.value.octets.length) {
            const value = isHuffman ? readHuffman(readingAcc.value.octets) : readingAcc.value.octets
          ...
          new Error('hpack headerFieldProcessor reading string error')
        }
        ...
        if (readingAcc.decoded.length === 0) {
          ...
          new Error('hpack headerFieldProcessor reading length error')
        }
        ...
        if (cur < 0x80 ) {
          ...
          const name = idx < staticTable.length ? staticTable[idx][0] : dynamicTable[idx - staticTable.length][0]
      ...
      if (cur > 0x80) {
        ...
        if (cur !== 0xff){
          ...
          const completePair = idx < staticTable.length ? staticTable[idx] : dynamicTable[idx - staticTable.length]
        ...
        if (cur !== 0x7f && cur !== 0x40){
          ...
          const headerFieldName = idx < staticTable.length ?  staticTable[idx][0] : dynamicTable[idx - staticTable.length]
        ...
        new Error('hpack headerFieldProcessor no initial header field match error')
      }
